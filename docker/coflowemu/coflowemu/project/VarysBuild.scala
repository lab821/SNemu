...
object VarysBuild extends Build {
  lazy val root = Project("root", file("."), settings = rootSettings) aggregate(core, services)

  lazy val core = Project("core", file("core"), settings = coreSettings)

  lazy val services = Project("services", file("services"), settings = servicesSettings) dependsOn (core)
  ...
  def servicesSettings = sharedSettings ++ Seq(
    name := "coflowemu-services"
  )
