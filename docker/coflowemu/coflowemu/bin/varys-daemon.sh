...
#   VARYS_MASTER    host:path where coflowemu code should be rsync'd from
#   VARYS_PID_DIR   The pid files are stored. /tmp by default.
#   VARYS_IDENT_STRING   A string representing this instance of coflowemu. $USER by default
#   VARYS_NICENESS The scheduling priority for daemons. Defaults to 0.
##

usage="Usage: coflowemu-daemon.sh [--config <conf-dir>] [--hosts hostlistfile] (start|stop) <coflowemu-command> <args...>"
...
# some variables
export VARYS_LOGFILE=coflowemu-$VARYS_IDENT_STRING-$command-$HOSTNAME.log
export VARYS_ROOT_LOGGER="INFO,DRFA"
log=$VARYS_LOG_DIR/coflowemu-$VARYS_IDENT_STRING-$command-$HOSTNAME.out
pid=$VARYS_PID_DIR/coflowemu-$VARYS_IDENT_STRING-$command.pid
