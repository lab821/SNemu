...
"$bin"/coflowemu-daemon.sh start coflowemu.framework.master.Master --ip $VARYS_MASTER_IP --port $VARYS_MASTER_PORT --webui-port $VARYS_MASTER_WEBUI_PORT

# Start a slave agent at the Master
sleep 1
"$bin"/coflowemu-daemon.sh start coflowemu.framework.slave.Slave coflowemu://$VARYS_MASTER_IP:$VARYS_MASTER_PORT
