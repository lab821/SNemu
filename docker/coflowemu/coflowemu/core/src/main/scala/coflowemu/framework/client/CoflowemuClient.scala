...
import coflowemu.framework.master.{Master, CoflowInfo}
import coflowemu.framework.slave.Slave
import coflowemu.util._

class CoflowemuClient(
    clientName: String,
    masterUrl: String,
  ...
  val INTERNAL_ASK_TIMEOUT_MS: Int = 
    System.getProperty("coflowemu.client.internalAskTimeoutMillis", "5000").toInt
  val RATE_UPDATE_FREQ = System.getProperty("coflowemu.client.rateUpdateIntervalMillis", "100").toLong
  val SHORT_FLOW_BYTES = System.getProperty("coflowemu.client.shortFlowMB", "1000").toLong * 1048576
  ...
  class ClientActor extends Actor with Logging {
    ...
    @throws(classOf[CoflowemuException])
    def masterDisconnected() {
      ...
      markDisconnected()
      context.stop(self)
      throw new CoflowemuException(connToMasterFailedMsg)
    }
    ...
    override def receive = {
      ...
      case StartEmuInTime(startTimeStamp) =>
        assert(listener.isInstanceOf[EmulationListener])
//        logInfo("Emulation will start at " + startTimeStamp)
        listener.asInstanceOf[EmulationListener].startEmulation(startTimeStamp)

      case LaunchTask(coflowId) =>
        assert(listener.isInstanceOf[EmulationListener])
        listener.asInstanceOf[EmulationListener].launchTask(coflowId)
    }
  ...
  def start() {
    ...
    val (actorSystem_, _) = AkkaUtils.createActorSystem("coflowemuClient", Utils.localIpAddress, 0)
  ...
  private def handlePutMultiple(
    ...
    if (dataType != DataType.FAKE) {
      ...
      logWarning(tmpM)
      throw new CoflowemuException(tmpM)
    }
    ...
    if (flowDesc.dataType == DataType.FAKE) {
      ...
      while (bytesReceived < flowDesc.sizeInBytes) {
        ...
        if (n == -1) {
          ...
          throw new CoflowemuException("Too few bytes received")
        } else {
          bytesReceived += n
        }
      }
        ...
        case Some(byteArr) => {
          ...
          flowDesc.dataType match {
            ...
            case _ => {
              ...
              throw new CoflowemuException("Invalid DataType!")
            }
          }
        }
        case None => {
