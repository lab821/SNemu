...
import coflowemu.ui.env.EnvironmentUI
import coflowemu.ui.JettyUtils._

/** Top level user interface for Varys */
private[coflowemu] class VarysUI() extends Logging {
  val host = Option(System.getenv("VARYS_PUBLIC_DNS")).getOrElse(Utils.localHostName())
  val port = Option(System.getProperty("coflowemu.ui.port")).getOrElse(VarysUI.DEFAULT_PORT).toInt
  ...
  private[coflowemu] def appUIAddress = host + ":" + boundPort.getOrElse("-1")

}

private[coflowemu] object VarysUI {
