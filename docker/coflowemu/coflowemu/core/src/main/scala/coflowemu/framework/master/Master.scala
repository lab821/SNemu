...
private[coflowemu] class Master(
    systemName:String, 
    actorName: String,
    host: String, 
    port: Int, 
  ...
  val NUM_MASTER_INSTANCES = System.getProperty("coflowemu.master.numInstances", "1").toInt
  val DATE_FORMAT = new SimpleDateFormat("yyyyMMddHHmmss")  // For coflow IDs
  val SLAVE_TIMEOUT = System.getProperty("coflowemu.slave.timeout", "60").toLong * 1000
  
  val CONSIDER_DEADLINE = System.getProperty("coflowemu.master.considerDeadline", "false").toBoolean

//  var traceHostNum = 0
//  var traceCoflowNum = 0
//  val traceArray = new ArrayBuffer[TracedCoflowInfo]()
  ...
  val schedulerClass = System.getProperty(
    "coflowemu.master.scheduler",
    "coflowemu.framework.master.scheduler.SEBFScheduler")

  val coflowScheduler = Class.forName(schedulerClass).newInstance.asInstanceOf[CoflowScheduler]

//  private def parseTrace(traceFile: String): Unit = {
//    val traceFileAdd = this.getClass.getResource("/") + traceFile
//    val traceSource = Source.fromFile(traceFileAdd.substring(5)).getLines() //  shen TM yao qu diao qian mian de "file:/"
//    logInfo("Found trace file at " + traceFileAdd)
//    if(traceSource.hasNext){
//      val tracePair = traceSource.next().split("\\s+")
//      traceHostNum = tracePair(0).toInt
//      traceCoflowNum = tracePair(1).toInt
...
//      val trace = new TracedCoflowInfo(rawCoflow(0).toInt, rawCoflow(1).toInt)
//      val numMapper = rawCoflow(2).toInt
//      var pos = 3
//      for(i <- 1 to numMapper){
//        trace.addMapper(rawCoflow(pos).toInt)
//        pos = pos + 1
//      }
//      val numReducer = rawCoflow(pos).toInt
//      pos = pos + 1
//      for(i <- 1 to numReducer){
//        trace.addReducer(rawCoflow(pos).toInt)
//        pos = pos + 1
//      }
//      for{x <- 1 to numReducer
//          y <- 1 to numMapper}{
//        trace.addTransfer(numMapper - 1, numReducer - 1, rawCoflow(pos).toDouble)
//        pos = pos + 1
//      }
//      traceArray += trace
//    }
...
//  parseTrace(traceFile)
  ...
  /**
    * 启动actorSystem以及MasterActor
    * @return actorSystem实例、绑定端口
    */
  ...
  private[coflowemu] class MasterActor(
      ip: String, 
      port: Int, 
      webUiPort: Int) 
    ...
    override def preStart() {
      logInfo("Starting Varys master at coflowemu://" + ip + ":" + port)
...
private[coflowemu] object Master {
  private val systemName = "coflowemuMaster"
  private val actorName = "Master"
  private val varysUrlRegex = "coflowemu://([^:]+):([0-9]+)".r
  ...
  /** 
   * Returns an `akka.tcp://...` URL for the Master actor given a coflowemuUrl `coflowemu://host:ip`.
   */
  def toAkkaUrl(varysUrl: String): String = {
    varysUrl match {
      ...
      case _ =>
        throw new CoflowemuException("Invalid master URL: " + varysUrl)
    }
  }
