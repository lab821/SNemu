...
import coflowemu.framework.{FlowDescription, CoflowDescription}

private[coflowemu] class CoflowInfo(
    val startTime: Long,
    val id: String,
  ...
  private val numAddedFlows = new AtomicInteger(0)
  def readyToLaunch = numAddedFlows.get() == desc.maxFlows
...
//      if(numFlowsToComplete == 0){
//        markFinished(CoflowState.FINISHED)
//      }
      true
    }
    false
