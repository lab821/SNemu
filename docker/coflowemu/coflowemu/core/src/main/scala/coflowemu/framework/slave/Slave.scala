package coflowemu.framework.slave
...
import coflowemu.framework.master.Master
import coflowemu.framework.slave.ui.SlaveWebUI
import coflowemu.{Logging, Utils, CoflowemuException}
import coflowemu.util._
import coflowemu.framework._

private[coflowemu] class SlaveActor(
    ip: String,
    port: Int,
    webUiPort: Int,
    commPort: Int,
  ...
  val HEARTBEAT_SEC = System.getProperty("coflowemu.framework.heartbeat", "1").toInt
  ...
  override def receive = {
    case RegisteredSlave(url) => {
      ...
      val sendStats = System.getProperty("coflowemu.slave.sendStats", "false").toBoolean
      if (sendStats) {
...
private[coflowemu] object Slave {
  ...
  /** 
   * Returns an `akka.tcp://...` URL for the Slave actor given a varysUrl `coflowemu://host:ip`.
   */
