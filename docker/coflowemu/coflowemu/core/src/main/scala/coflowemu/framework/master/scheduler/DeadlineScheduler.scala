package coflowemu.framework.master.scheduler

import scala.collection.mutable.{ArrayBuffer, Map}

import coflowemu.framework.master.{CoflowInfo, CoflowState, FlowInfo, SlaveInfo}
import coflowemu.Logging
...
class DeadlineScheduler extends OrderingBasedScheduler with Logging {
  ...
  val CONSIDER_DEADLINE = System.getProperty("coflowemu.master.consdierDeadline", "false").toBoolean
  val DEADLINE_PAD = System.getProperty("coflowemu.master.deadlinePadding", "0.1").toDouble
  val MIN_DEADLINE = System.getProperty("coflowemu.master.minDeadlineMillis", "200").toInt

  if (!CONSIDER_DEADLINE) {
    logError("coflowemu.master.consdierDeadline must be true for DeadlineScheduler")
    System.exit(1)
  }
